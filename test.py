...
def denoise(data,pred):
	...
	reduced_noise = nr.reduce_noise(y=data, sr=sr1, y_noise=noise)
	print(reduced_noise)
	sf.write('clean.wav', reduced_noise * 5, sr2)
...
chroma_cq = np.mean(librosa.feature.chroma_cqt(y=y, sr=sr,n_chroma=40,bins_per_octave=None).T,axis=0)
chroma_cens = np.mean(librosa.feature.chroma_cens(y=y, sr=sr,n_chroma=40,bins_per_octave=None).T,axis=0)
